#!/usr/bin/env python3
"""
AgenticRAG with Markov Chain Anomaly Detection for CERT Insider Threat Dataset

This script demonstrates the integration of AgenticRAG with the Markov chain
anomaly detection agent for analyzing the CERT insider threat dataset.
"""
...
class AgenticRAGCertTest:
    """
    Test framework for AgenticRAG with Markov chain anomaly detection on CERT dataset
    """
    ...
    def create_enhanced_cert_dataset(self):
        ...
        # Add derived features for AgenticRAG
        df = self._add_agentic_features(df)
        
        # Save dataset
        os.makedirs(self.dataset_path, exist_ok=True)
        df.to_csv(f"{self.dataset_path}/agentic_cert_dataset.csv", index=False)
        
        print(f"   ✅ Created AgenticRAG CERT dataset with {len(df)} records")
        print(f"   ✅ {len(malicious_users)} malicious users identified")
        print(f"   ✅ {df['is_malicious'].sum()} malicious activity records")
        
        return df
    
    def _add_agentic_features(self, df: pd.DataFrame):
        """Add features optimized for AgenticRAG analysis"""
        # Time-based features
    ...
    def prepare_data_for_agentic_rag(self, df: pd.DataFrame, time_interval: int = 60):
        ...
        for idx, row in df_resampled.iterrows():
            agentic_data.append({
                ...
                'user_id': 'aggregated',  # For aggregated analysis
...
async def main():
    ...
    print("🤖 AgenticRAG with Markov Chain Anomaly Detection")
    print("📊 CERT Insider Threat Dataset Analysis")
    print("=" * 60)
    
    # Initialize test framework
    test_framework = AgenticRAGCertTest()
