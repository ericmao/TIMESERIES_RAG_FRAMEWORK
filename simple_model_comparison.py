...
This script demonstrates model comparison using available models.
"""
...
class SimpleModelComparisonTest:
    """Simple model comparison test using available models"""
    ...
    async def run_comparison(self):
        ...
        print("🚀 Starting Simple Model Comparison Test")
        print("=" * 50)
        
        # Test DialoGPT Medium
    ...
    def _generate_report(self):
        ...
        if "dialo_gpt_medium" in metrics and "dialo_gpt_large" in metrics:
            print(f"\n📈 Improvements (Large vs Medium):")
            ...
            time_improvement = safe_improvement(
                metrics["dialo_gpt_large"]["total_time"], 
                metrics["dialo_gpt_medium"]["total_time"]
            )
            forecasting_improvement = safe_improvement(
                metrics["dialo_gpt_large"]["avg_forecasting_confidence"], 
                metrics["dialo_gpt_medium"]["avg_forecasting_confidence"]
            )
            anomaly_improvement = safe_improvement(
                metrics["dialo_gpt_large"]["avg_anomaly_confidence"], 
                metrics["dialo_gpt_medium"]["avg_anomaly_confidence"]
            )
            classification_improvement = safe_improvement(
                metrics["dialo_gpt_large"]["avg_classification_confidence"], 
                metrics["dialo_gpt_medium"]["avg_classification_confidence"]
            )
    ...
    def _create_visualizations(self):
        ...
        fig.suptitle('DialoGPT Large vs Medium Model Comparison', fontsize=16)
        ...
        plt.tight_layout()
        plt.savefig('simple_model_comparison_results.png', dpi=300, bbox_inches='tight')
        print("  ✅ Visualization saved as 'simple_model_comparison_results.png'")
        
        # Save detailed results
        with open('simple_model_comparison_results.json', 'w') as f:
            json.dump(self.results, f, indent=2, default=str)
        print("  ✅ Detailed results saved as 'simple_model_comparison_results.json'")

async def main():
    """Main function"""
    print("🎯 DialoGPT Large vs Medium Model Comparison Test")
    print("=" * 60)
    
    # Create test instance
    test = SimpleModelComparisonTest()
    ...
    print("  - simple_model_comparison_results.png (visualizations)")
    print("  - simple_model_comparison_results.json (detailed data)")
